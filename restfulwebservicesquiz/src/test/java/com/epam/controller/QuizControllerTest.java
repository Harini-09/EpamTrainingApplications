...
class QuizControllerTest {
	...
	void testCreateQuiz() throws Exception {
		Mockito.when(quizService.insert(quizDto)).thenReturn(quizDto);
	...
	void addQuestionTestSuccess() throws Exception {
		...
		Mockito.when(quizService.addQuestion(1, 1)).thenReturn(quizDto);
	...
	void removeQuestionTest() throws Exception {
		...
		Mockito.when(quizService.removeQuestion(1, 1)).thenReturn(quizDto);
	...
	void modifyMarksTestSuccess() throws Exception {
		doNothing().when(quizService).isQuizTitlePresent(1);
		Mockito.when(quizService.updateMarks(1, 30)).thenReturn(quizDto);
	...
	void deleteQuizSuccess() throws Exception {
		doNothing().when(quizService).isQuizTitlePresent(1);
		Mockito.when(quizService.delete(1)).thenReturn(quizDto);
